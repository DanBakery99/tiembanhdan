import{i as g}from"./data-EdnpEie_.js";const i=()=>JSON.parse(JSON.stringify(g)),v=()=>{const t=localStorage.getItem("siteData");if(!t)return i();try{const e=JSON.parse(t);return e.hero||(e.hero=i().hero),e.menu||(e.menu=i().menu),e.gallery||(e.gallery=i().gallery),e.contact||(e.contact=i().contact),e}catch{return i()}};let n=v();const d=t=>new Promise((e,a)=>{const o=new FileReader;o.readAsDataURL(t),o.onload=r=>{const l=new Image;l.src=r.target.result,l.onload=()=>{try{const c=document.createElement("canvas"),m=1600;if(!l.width||!l.height)return a(new Error("Invalid image dimensions"));const p=l.width>m?m/l.width:1;c.width=Math.round(l.width*p),c.height=Math.round(l.height*p),c.getContext("2d").drawImage(l,0,0,c.width,c.height),e(c.toDataURL("image/jpeg",.85))}catch(c){a(c)}},l.onerror=c=>a(c)},o.onerror=r=>a(r)}),b=document.getElementById("login-modal"),f=document.getElementById("admin-dashboard"),w=document.getElementById("login-form"),x=document.getElementById("login-error");w.addEventListener("submit",t=>{t.preventDefault(),document.getElementById("password").value==="Toobakery0810"?(b.classList.add("hidden"),f.classList.remove("hidden"),E()):x.classList.remove("hidden")});document.getElementById("logout-btn").addEventListener("click",()=>{location.reload()});const h=document.querySelectorAll(".tab-btn"),I=document.querySelectorAll(".tab-content");h.forEach(t=>{t.addEventListener("click",()=>{h.forEach(a=>{a.classList.remove("bg-primary","text-white","shadow-lg"),a.classList.add("text-secondary","hover:bg-white","hover:shadow-md")}),t.classList.remove("text-secondary","hover:bg-white","hover:shadow-md"),t.classList.add("bg-primary","text-white","shadow-lg"),I.forEach(a=>a.classList.add("hidden"));const e=document.getElementById(`tab-${t.dataset.tab}`);e&&e.classList.remove("hidden")})});const E=()=>{y(),u(),s(),B(),k()},y=()=>{n.hero||(n.hero=g.hero),n.hero.backgroundImages||(n.hero.backgroundImages=g.hero.backgroundImages);const t=n.hero.backgroundImages,e=document.getElementById("hero-preview-1"),a=document.getElementById("hero-preview-2"),o=document.getElementById("hero-url-1"),r=document.getElementById("hero-url-2");e&&t[0]&&(e.src=t[0].src),a&&t[1]&&(a.src=t[1].src),o&&t[0]&&(o.value=t[0].src),r&&t[1]&&(r.value=t[1].src)};window.updateHeroImage=(t,e)=>{n.hero.backgroundImages[t]?n.hero.backgroundImages[t].src=e:n.hero.backgroundImages[t]={src:e,label:""},y()};window.uploadHeroImage=async(t,e)=>{if(e.files&&e.files[0])try{const a=await d(e.files[0]);window.updateHeroImage(t,a)}catch(a){alert("Lỗi upload ảnh: "+a.message)}};const u=()=>{const t=document.getElementById("menu-list");t.innerHTML="",n.menu.items.forEach((e,a)=>{const o=document.createElement("div");o.className="bg-surface p-6 rounded-[2rem] flex flex-col md:flex-row gap-6 items-start border border-primary/5 shadow-sm hover:shadow-md transition-all group",o.innerHTML=`
            ...
            <div class="flex-1 space-y-4 w-full">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <input type="text" value="${e.name}" placeholder="Tên món" class="w-full px-4 py-2 bg-white border border-primary/5 rounded-xl text-sm font-bold focus:ring-2 focus:ring-accent outline-none" onchange="updateMenuItem(${a}, 'name', this.value)">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[9px] font-bold text-primary/30 uppercase tracking-tighter ml-1">Giá Tiền</label>
                        <input type="text" value="${e.price}" placeholder="Giá" class="w-full px-4 py-2 bg-white border border-primary/5 rounded-xl text-sm focus:ring-2 focus:ring-accent outline-none" onchange="updateMenuItem(${a}, 'price', this.value)">
                    </div>
                </div>
                <div class="space-y-1">
                    <label class="text-[9px] font-bold text-primary/30 uppercase tracking-tighter ml-1">Mô Tả</label>
                    <input type="text" value="${e.description}" placeholder="Mô tả" class="w-full px-4 py-2 bg-white border border-primary/5 rounded-xl text-sm focus:ring-2 focus:ring-accent outline-none" onchange="updateMenuItem(${a}, 'description', this.value)">
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                     <div class="md:col-span-2 space-y-1">
                        <label class="text-[9px] font-bold text-primary/30 uppercase tracking-tighter ml-1">URL Hình Ảnh</label>
                        <input type="text" value="${e.image}" placeholder="URL Hình ảnh" class="w-full px-4 py-2 bg-white border border-primary/5 rounded-xl text-[10px] text-gray-500 focus:ring-2 focus:ring-accent outline-none" onchange="updateMenuItem(${a}, 'image', this.value)">
                     ...
                     <div class="space-y-1">
                        ...
                        <input type="text" value="${e.tag||""}" placeholder="Tag" class="w-full px-4 py-2 bg-white border border-primary/5 rounded-xl text-sm focus:ring-2 focus:ring-accent outline-none" onchange="updateMenuItem(${a}, 'tag', this.value)">
                     </div>
                </div>
        ...
        `,t.appendChild(o)})};window.updateMenuItem=(t,e,a)=>{n.menu.items[t][e]=a};window.uploadMenuImage=async(t,e)=>{if(e.files&&e.files[0])try{const a=await d(e.files[0]);n.menu.items[t].image=a,u()}catch(a){alert("Lỗi upload ảnh: "+a.message)}};window.deleteMenuItem=t=>{confirm("Bạn có chắc muốn xóa món này?")&&(n.menu.items.splice(t,1),u())};window.addMenuItem=()=>{n.menu.items.push({id:Date.now(),name:"Món Mới",description:"Mô tả món ăn...",price:"Liên hệ",image:"/assets/img1.png",tag:"",tagColor:"primary",category:"bánh ngọt"}),u()};const s=()=>{const t=document.getElementById("gallery-list");t.innerHTML="";const e=document.createElement("div");e.className="flex flex-col items-center justify-center bg-surface border-2 border-dashed border-primary/10 rounded-2xl aspect-square cursor-pointer hover:bg-white hover:border-accent/40 transition-all group",e.onclick=()=>document.getElementById("gallery-upload-input").click(),e.innerHTML=`
    ...
    `,t.appendChild(e),n.gallery.images.forEach((a,o)=>{const r=document.createElement("div");r.className="relative group aspect-square rounded-2xl overflow-hidden border border-primary/5 shadow-sm hover:shadow-xl transition-all",r.innerHTML=`
            ...
            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3">
                ...
                <input type="text" value="${a}" class="w-full p-2 text-[9px] bg-white/90 rounded-lg outline-none backdrop-blur-sm" onchange="updateGalleryItem(${o}, this.value)">
            </div>
        `,t.appendChild(r)})};window.uploadGalleryImage=async t=>{if(t.files&&t.files[0])try{const e=await d(t.files[0]);n.gallery.images.push(e),s()}catch(e){alert("Lỗi upload ảnh: "+e.message)}};window.deleteGalleryItem=t=>{confirm("Xóa ảnh này?")&&(n.gallery.images.splice(t,1),s())};window.updateGalleryItem=(t,e)=>{n.gallery.images[t]=e,s()};window.addGalleryItem=()=>{const t=prompt("Nhập URL hình ảnh (hoặc dùng nút Upload):");t&&(n.gallery.images.push(t),s())};const B=()=>{document.getElementById("about-title-input").value=n.about.title,document.getElementById("about-image-input").value=n.about.image,document.getElementById("about-content-input").value=n.about.paragraphs.join(`
...
`).filter(a=>a.trim()!=="");const t=document.getElementById("about-image-file");t&&(t.onchange=async e=>{if(e.target.files&&e.target.files[0])try{const a=await d(e.target.files[0]);n.about.image=a,document.getElementById("about-image-input").value=a,alert("Đã cập nhật ảnh thành công!")}catch(a){alert("Lỗi: "+a)}})},k=()=>{document.getElementById("contact-address-input").value=n.contact.address||"",document.getElementById("contact-phone-input").value=n.contact.phone||"",document.getElementById("contact-hours-input").value=Array.isArray(n.contact.hours)?n.contact.hours.join(", "):n.contact.hours||"",document.getElementById("contact-map-input").value=n.contact.mapEmbed||"",document.getElementById("contact-google-rating-input").value=n.contact.googleRating||5,document.getElementById("contact-google-count-input").value=n.contact.googleReviewCount||0,document.getElementById("contact-maps-url-input").value=n.contact.googleMapsUrl||"",document.getElementById("contact-zalo-input").value=n.contact.zaloUrl||"",document.getElementById("contact-facebook-input").value=n.contact.facebookUrl||"",document.getElementById("contact-instagram-input").value=n.contact.instagramUrl||"",document.getElementById("contact-address-input").onchange=e=>n.contact.address=e.target.value,document.getElementById("contact-phone-input").onchange=e=>n.contact.phone=e.target.value,document.getElementById("contact-hours-input").onchange=e=>n.contact.hours=e.target.value.split(",").map(a=>a.trim()).filter(a=>a!==""),document.getElementById("contact-map-input").onchange=e=>n.contact.mapEmbed=e.target.value,document.getElementById("contact-google-rating-input").onchange=e=>n.contact.googleRating=parseFloat(e.target.value),document.getElementById("contact-google-count-input").onchange=e=>n.contact.googleReviewCount=parseInt(e.target.value),document.getElementById("contact-maps-url-input").onchange=e=>n.contact.googleMapsUrl=e.target.value,document.getElementById("contact-zalo-input").onchange=e=>n.contact.zaloUrl=e.target.value,document.getElementById("contact-facebook-input").onchange=e=>n.contact.facebookUrl=e.target.value,document.getElementById("contact-instagram-input").onchange=e=>n.contact.instagramUrl=e.target.value;const t=document.getElementById("admin-reviews-list");if(t){for(n.contact.reviews||(n.contact.reviews=[]);n.contact.reviews.length<3;)n.contact.reviews.push({name:"Khách hàng",rating:5,text:"",date:"vừa xong"});t.innerHTML=n.contact.reviews.map((e,a)=>`
            <div class="p-6 bg-white border border-primary/5 rounded-[1.5rem] space-y-4 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <input type="text" value="${e.name}" placeholder="Tên khách" class="w-full px-4 py-2 bg-surface border border-primary/5 rounded-xl text-sm font-bold focus:ring-2 focus:ring-accent outline-none" onchange="appData.contact.reviews[${a}].name = this.value">
                    </div>
                    <div class="space-y-1">
        ...
        `).join("")}};document.getElementById("save-btn").addEventListener("click",()=>{try{localStorage.setItem("siteData",JSON.stringify(n)),alert("Đã lưu thay đổi vào trình duyệt!")}catch(t){t.name==="QuotaExceededError"?alert("Lỗi: Dung lượng lưu trữ đã đầy! Hãy thử xóa bớt ảnh hoặc dùng ảnh nhỏ hơn."):alert("Lỗi khi lưu: "+t.message)}});document.getElementById("reset-btn").addEventListener("click",()=>{confirm("Bạn có chắc muốn reset về mặc định không? Mọi thay đổi chưa xuất file sẽ mất.")&&(localStorage.removeItem("siteData"),location.reload())});document.getElementById("export-btn").addEventListener("click",()=>{const t="export const initialData = "+JSON.stringify(n,null,4)+";",e=new Blob([t],{type:"text/javascript;charset=utf-8"}),a=URL.createObjectURL(e),o=document.createElement("a");o.href=a,o.download="data.js",document.body.appendChild(o),o.click(),document.body.removeChild(o)});
